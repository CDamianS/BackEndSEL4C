...
def existe_admin(request):
    """Revisa si el administrador existe en la base de datos."""
...
def mandar_respuestas(request):
    if request.method == 'POST':
        try:
            ...
            elUsuario = Usuario.objects.get(usuarioID = usuarioID)
            resultados = CuestionarioInicial.objects.filter(usuarioID = elUsuario).values()

            for resultado in resultados:
                usuario = resultado['usuarioID_id']
                pregunta = resultado['numero']
        ...
        return HttpResponse('metdodo invalido')

#Pregntas para cuestionario
@csrf_exempt
>>>>>>> Stashed changes
